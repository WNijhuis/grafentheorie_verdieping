...
\end{stelling}
De kleinste \(N\) die hieraan voldoet wordt het \textit{Ramsey getal} van \(G\) genoemd en genoteerd als \(R(G)\). Als \(G=K_t\), dan schrijven we \(R(t)\). Meer algemeen is \(R(t_1, \dots, t_k, k)\) de kleinste \(N\in\set{N}\) zodat elke \(k\)-kleuring van \(K_N\) een monochromatische kopie van \(K_{t_a}\) bevat met kleur \(a\) \cite{conlon2015}.
...
In 1935 bewezen Paul Erd\H{o}s en Georges Szekeres de volgende twee stellingen m.b.t. het vinden van een algemene bovengrens voor Ramsey-getallen \cite{ma2025}.

\begin{stelling}
    \(\displaystyle R(l,k) \leq \binom{k+l-2}{l-1}\)
\end{stelling}
\begin{stelling}
    \(\displaystyle R(l) \leq 4^l\)
\end{stelling}
